...
const HIDDEN_MARKETPLACE_EXTENSION_IDS = new Set(["coodi.tsx"]);
...
export function mergeMarketplaceLanguageExtensions(
  ...
  const typescript = visibleExtensions.find((manifest) => manifest.id === "coodi.typescript");
  const tsx = extensions.find((manifest) => manifest.id === "coodi.tsx");
