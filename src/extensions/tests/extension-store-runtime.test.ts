...
describe("extension-store runtime manifest", () => {
  it("uses the known working Marksman release asset instead of stale extension package URLs", () => {
    const url = resolveToolDownloadUrlForManifest(
      {
        name: "marksman",
        downloadUrl: "https://www.mubashirhassan.com/coodi/extensions/markdown/markdown-${os}-${arch}.tar.gz",
      },
      "1.0.0",
    );
    ...
    const url = resolveToolDownloadUrlForManifest(
      {
        ...
        downloadUrl: "https://www.mubashirhassan.com/coodi/extensions/packages/lua/lua-${os}-${arch}.tar.gz",
      },
      "1.0.0",
    );
  ...
  it("defers generic platform URL templates to the Rust backend for libc-aware resolution", () => {
    const template =
      "https://www.mubashirhassan.com/coodi/extensions/test/test-${targetArch}-${targetOs}.${archiveExt}";
  ...
  it("rewrites managed tool commands to resolved paths", () => {
    ...
    const runtimeManifest = buildRuntimeManifest(manifest, {
      lsp: "/tmp/coodi-tools/bin/marksman",
    });

    expect(runtimeManifest.lsp?.server.default).toBe("/tmp/coodi-tools/bin/marksman");
  });
  ...
  it("preserves companion packages when rewriting managed LSP commands", () => {
    ...
    const runtimeManifest = buildRuntimeManifest(manifest, {
      lsp: "/tmp/coodi-tools/bun/@vtsls/language-server/node_modules/@vtsls/language-server/bin/vtsls.js",
    });
