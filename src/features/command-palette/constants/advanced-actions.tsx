...
import { openCoodiLogBuffer } from "@/features/settings/services/coodi-log-service";
...
export const createAdvancedActions = (params: AdvancedActionsParams): Action[] => {
  ...
  const baseActions: Action[] = [
    ...
    {
      id: "developer-open-coodi-log",
      label: "Developer: Open Coodi Log",
      description: "Open the current Coodi application log in a read-only editor tab",
      icon: <Terminal />,
      category: "Developer",
      action: async () => {
        try {
          await openCoodiLogBuffer();
        } catch (error) {
          showToast({
            message: error instanceof Error ? error.message : "Failed to open Coodi log",
            type: "error",
          });
        }
        onClose();
    ...
    },
    {
      ...
      description: "Install 'coodi' command for terminal",
      icon: <Terminal />,
      category: "CLI",
      action: async () => {
        try {
