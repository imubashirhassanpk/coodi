...
export const buildContextPrompt = (context: ContextInfo): string => {
  ...
  if (isAcpAgent) {
    contextPrompt += `Coodi ACP Extension Methods (protocol methods, NOT shell commands):
- Call \`coodi.openWebViewer\` with \`{ "url": "https://..." }\` to open websites inside Coodi.
- Call \`coodi.openTerminal\` with \`{ "command": "..." }\` to open a terminal tab in Coodi.
- Call \`coodi.setChatTitle\` with \`{ "title": "Short title" }\` to rename the active Coodi chat.
- Do NOT run \`ext_method\` in a terminal.
- Do NOT use shell/browser commands like \`open https://...\` for "open on web" requests; use \`coodi.openWebViewer\` instead.
...
export const buildSystemPrompt = (
  ...
  const hasAcpExtensions = contextPrompt.includes("Coodi ACP Extension Methods");
...
ACP extension rules:
- Use Coodi extension methods as protocol calls, not shell commands.
- Never run \`ext_method\` in a terminal command.
- For "open URL/web/site" requests, call \`coodi.openWebViewer\` directly instead of suggesting \`open https://...\`.
- For "open X in terminal" requests (for example lazygit), call \`coodi.openTerminal\` with \`{ "command": "X" }\`.
- For rename/title requests, call \`coodi.setChatTitle\` with \`{ "title": "Short title" }\`.
- Never say Coodi extension methods are unavailable or require MCP exposure in this ACP session.
- After calling an Coodi extension method, confirm success and stop; do not retry with shell fallbacks unless user asks.`;
  }
