use super::{
   AcpConnection,
   client::{CoodiAcpClient, PermissionResponse},
...
use coodi_terminal::TerminalManager;
use serde_json::json;
use std::{
...
pub(super) async fn initialize_worker(
   ...
   let client = Arc::new(CoodiAcpClient::new(
      app_handle.clone(),
...
async fn initialize_connection(
   ...
   client_meta.insert(
      ...
      json!({
         "extensionMethods": [
            { "name": "coodi.openWebViewer", "description": "Open a URL in Coodi web viewer", "params": { "url": "string" } },
            { "name": "coodi.openTerminal", "description": "Open a terminal tab in Coodi", "params": { "command": "string|null" } },
            { "name": "coodi.setChatTitle", "description": "Rename the active Coodi chat title", "params": { "title": "string" } }
         ],
   ...
   let init_request = acp::InitializeRequest::new(acp::ProtocolVersion::LATEST)
      ...
      .client_info(acp::Implementation::new("coodi", env!("CARGO_PKG_VERSION")).title("Coodi"));
