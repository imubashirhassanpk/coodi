...
impl NodeRuntime {
   ...
   /// 2. Check if Coodi-managed Node.js exists
   ...
   pub async fn get_or_install(managed_root: Option<&Path>) -> Result<Self, RuntimeError> {
      ...
      if let Ok(runtime) = Self::from_managed_path(&managed_dir) {
         log::info!("Using Coodi-managed Node.js at {:?}", runtime.binary_path);
         return Ok(runtime);
      }
   ...
   /// Get Node.js runtime, preferring an existing Coodi-managed runtime over
