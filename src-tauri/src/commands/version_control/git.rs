use coodi_version_control::git as git_backend;
...
fn resolve_backend_path(path: String) -> String {
   coodi_wsl::resolve_windows_path(&path).unwrap_or(path)
}

fn restore_provider_path(original_path: &str, backend_path: String) -> String {
   if coodi_wsl::is_wsl_path(original_path) {
      coodi_wsl::windows_unc_to_wsl_uri(&backend_path).unwrap_or(backend_path)
   } else {
      backend_path
   }
}
